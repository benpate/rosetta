...
func TestMapOfString_GroupFooter_FirstElementClosesItsGroup(t *testing.T) {

	// A single element is both the header AND the footer of its group
	single := MapOfString{{"group": "a"}}.GroupBy("group")
	...
	pair := MapOfString{{"group": "a"}, {"group": "b"}}.GroupBy("group")
