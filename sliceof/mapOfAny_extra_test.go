...
func testMapOfAny() MapOfAny {
	return MapOfAny{
		mapof.Any{"name": "alpha"},
		mapof.Any{"name": "bravo"},
		mapof.Any{"name": "charlie"},
	}
}

func TestMapOfAny_Manipulations(t *testing.T) {
...
func TestMapOfAny_EmptyAccessors(t *testing.T) {
	x := NewMapOfAny()
...
func TestMapOfAny_FindFilterContains(t *testing.T) {

	x := testMapOfAny()

	found, ok := x.Find(func(v mapof.Any) bool { return v.GetString("name") == "bravo" })
	require.True(t, ok)
	require.Equal(t, "bravo", found.GetString("name"))

	_, ok = x.Find(func(v mapof.Any) bool { return v.GetString("name") == "missing" })
	require.False(t, ok)

	filtered := x.Filter(func(v mapof.Any) bool { return v.GetString("name") != "bravo" })
	require.Equal(t, 2, filtered.Length())

	require.True(t, x.Contains(func(v mapof.Any) bool { return v.GetString("name") == "alpha" }))
	require.False(t, x.Contains(func(v mapof.Any) bool { return v.GetString("name") == "missing" }))
}

func TestMapOfAny_ReverseRange(t *testing.T) {

	x := testMapOfAny()
	require.Equal(t, "charlie", x.Reverse().First().GetString("name"))

	collected := make([]string, 0)
	for _, value := range testMapOfAny().Range() {
		collected = append(collected, value.GetString("name"))
	}
	require.Equal(t, []string{"alpha", "bravo", "charlie"}, collected)
}

func TestMapOfAny_AppendShuffleKeys(t *testing.T) {

	x := NewMapOfAny()
	x.Append(mapof.Any{"name": "x"}, mapof.Any{"name": "y"})
	require.Equal(t, 2, x.Length())

	shuffled := testMapOfAny().Shuffle()
	require.Equal(t, 3, shuffled.Length())

	require.Equal(t, []string{"0", "1"}, x.Keys())
}

func TestMapOfAny_GettersSetters(t *testing.T) {

	x := testMapOfAny()

	value, ok := x.GetAnyOK("1")
	require.True(t, ok)
	require.Equal(t, "bravo", value.(mapof.Any).GetString("name"))
	...
	require.True(t, x.SetIndex(5, map[string]any{"name": "grown"}))
	require.Equal(t, "grown", x.At(5).GetString("name"))
}

func TestMapOfAny_SetValueRemove(t *testing.T) {

	x := NewMapOfAny()
	require.NoError(t, x.SetValue(testMapOfAny()))
	require.Equal(t, 3, x.Length())

	// A non-MapOfAny value is rejected
