...
- **`FromText` escapes `&` FIRST, before the other entity replacements.** Order is important: escaping `<`/`>`/`"`/`'` introduces `&…;` entities, so if `&` were escaped after them it would double-escape those entities into `&amp;lt;`. Don't reorder the `ReplaceAll` chain in `fromText.go`.
