...
func TestIntPointer(t *testing.T) {
	...
	require.NoError(t, schema.Set(&getter, "value", int(1)))
	require.Equal(t, int(100), getter.value)
	_, _, err = Validate(schema, &getter)
	require.NoError(t, err)

	// Value above the maximum is clamped to the maximum (does not fail)
	require.NoError(t, schema.Set(&getter, "value", int(1000000000)))
	require.Equal(t, int(1000000), getter.value)
	_, _, err = Validate(schema, &getter)
	require.NoError(t, err)
}
