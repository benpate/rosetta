...
func (schema Schema) get(object any, element Element, path list.List) (any, error) {
	...
	if !ok {
		return nil, derp.InternalError(location, "Unknown property", head)
	}
	...
	case Any, Array, Object:
		...
		if !ok {
			return nil, derp.InternalError(location, "Object must be a PointerGetter", object)
		}
	...
	case Boolean:
		...
		return nil, derp.InternalError(location, "Unable to get bool from BoolGetter or a PointerGettter", object)
	...
	case Integer:
		...
		if typed.BitSize == 64 {
			...
			return nil, derp.InternalError(location, "Unable to get int64 from Int64Getter or PointerGetter", object)
		}
		...
		return nil, derp.InternalError(location, "Unable to get int from IntGetter or PointerGetter", object)
	...
	case Number:
		...
		return nil, derp.InternalError(location, "Unable to get float from FloatGetter or PointerGetter", object)
	...
	case String:
		...
		return nil, derp.InternalError(location, "Unable to get string from StringGetter or PointerGetter", object)
	}

	return nil, derp.InternalError(location, "Unable to get property", path, object)
}
